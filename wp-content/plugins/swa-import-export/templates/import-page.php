...
$current_demo_installed = get_option('swa_ie_demo_installed', '');
...
$php_ver = PHP_VERSION;
$_notice = ($memory_limit < 128 || ($max_time < 60 && $max_time !==0 )|| $post_max_size < 32) ? 'swa-ie-warning' : 'swa-ie-good';
...
<div class="wrap">
    <div class="swa-ie-dashboard">
        <div class="swa-field-info <?php echo esc_attr($_notice); ?>">
            ...
            <div class="swa-advance-options">
                <ul class="swa-options">
                    <li class="swa-show-manual-import"><span
                                class="dashicons dashicons-media-spreadsheet"></span><?php echo esc_html__("Manual Import", SWA_TEXT_DOMAIN) ?>
                    </li>
                    <li class="swa-show-regenerate-thumbnail"><span
                                class="dashicons dashicons-images-alt"></span><?php echo esc_html__("Regenerate Thumbnails", SWA_TEXT_DOMAIN) ?>
                    </li>
                    <li class="swa-advance-reset"><span
                                class="dashicons dashicons-update"></span><?php echo esc_html__("Reset Site", SWA_TEXT_DOMAIN) ?>
                    </li>
                </ul>
                <span class="dashicons dashicons-admin-generic"></span>
                <form method="post" class="swa-reset-form-advance" style="display: none">
                    <?php wp_nonce_field('swa-reset', '_wp_nonce'); ?>
                    <input type="hidden" name="action" value="swa-reset">
                </form>
                <form method="post" class="swa-regenerate-thumbnail-sm" style="display: none">
                    <input type="hidden" name="action" value="swa-regenerate-thumbnails">
                </form>
            </div>
            <?php
            include_once swa_ie()->plugin_dir . 'templates/manual-import-template.php';
            ?>
        </div>
