...
class CMS_Post_Metabox
{
    ...
    protected function optimize_default_args()
    {
        ...
        $this->default_args['display_name'] = esc_html__('Settings', ETC_TEXT_DOMAIN);
    ...
    function add_section($post_type, $section = array())
    {
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$post_type])) {
                trigger_error(esc_html__('The field with id', ETC_TEXT_DOMAIN) . ' ' . esc_html($field['id']) . ' ' . esc_html__('for post type', ETC_TEXT_DOMAIN) . ' ' . esc_html($post_type) . ' ' . esc_html__('is already registered.', ETC_TEXT_DOMAIN));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $post_type)) {
                trigger_error(esc_html__('Section', ETC_TEXT_DOMAIN) . ' ' . esc_html($field['id']) . ' ' . esc_html__('for post type', ETC_TEXT_DOMAIN) . ' ' . esc_html($post_type) . ' ' . esc_html__('is already registered.', ETC_TEXT_DOMAIN));
...
//        die();
        ...
        wp_enqueue_style('cms-metabox', ETC_URL . '/assets/css/metabox' . Redux_Functions::isMin() . '.css', array(), self::$version, 'all');
        wp_enqueue_script('cms-metabox', ETC_URL . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array(
            'jquery',
            'redux-js'
        ), self::$version, 'all');
    ...
    {
        return ETC_PATH . '/templates/panel-post-meta';
    }
    ...
    function admin_notices()
    {
        ...
        if (!empty($this->notices['errors'])) {
            ...
            printf(
                ...
                esc_html__('error(s)', ETC_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', ETC_TEXT_DOMAIN)
            );
            echo '</p></div>';
        }
        ...
        if (!empty($this->notices['warnings'])) {
            ...
            printf(
                ...
                esc_html__('warning(s)', ETC_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', ETC_TEXT_DOMAIN)
            );
