...
if(!function_exists('etc_print_html')){
    function etc_print_html($content){
        echo $content;
    }
}

if(!function_exists('etc_print_shortcode')) {
    function etc_print_shortcode($content)
    {
        echo do_shortcode($content);
    }
}

if(!function_exists('etc_register_shortcode')) {
    function etc_register_shortcode($tag, $callback)
    {
...
if(!function_exists('etc_register_wp_widget')) {
    function etc_register_wp_widget($class)
    {
...
if(!function_exists('etc_get_template') && !function_exists('etc_get_locate_template')){
    function etc_get_template($widget, $template_path = '', $default_path = ''){
        ...
        $located = etc_get_locate_template($template_name, $layout, $template_path, $default_path);
        ...
        $located = apply_filters('etc_template_part', $located, $template_name, $settings, $template_path, $default_path);

        include($located);

    }

    function etc_get_locate_template($template_name, $layout = '1', $template_path = '', $default_path = '')
    {
        $layout_name = 'layout' . $layout . '.php';
        if (!$template_path) {
            $template_path = apply_filters('etc_template_path', 'elementor/templates/widgets/' . $template_name . '/');
        }

        if (!$default_path) {
            $default_path = ETC_PATH . 'templates/widgets/' . $template_name . '/';
        }
        ...
        return apply_filters('etc_locate_template', $template, $template_name, $template_path);
    }
}

if(!function_exists('etc_get_post_types')){
    function etc_get_post_types(){

    }
}
if(!function_exists('etc_get_post_type_options')){
    function etc_get_post_type_options(){
...
if(!function_exists('etc_get_post_categories')){
    function etc_get_post_categories(){
        return get_terms('category', array(
            'hide_empty' => false,
            'order' => 'desc',
        ));
    }
    if(!function_exists('etc_get_post_categories_options')){
        function etc_get_post_categories_options(){
            $categories = etc_get_post_categories();
            $options = array();
...
if(!function_exists('etc_get_element_id')){
    function etc_get_element_id($settings){
        return $settings['element_name'] . '-' . $settings['element_id'];
    }
}

if(!function_exists('etc_get_grid_term_list')){
    function etc_get_grid_term_list($post_type, $taxonomy = array())
    {
        if (empty($taxonomy)) {
...
if(!function_exists('etc_get_grid_term_options')){
    function etc_get_grid_term_options($post_type, $taxonomy = array())
    {
        if (empty($taxonomy)) {
            $taxonomy = get_object_taxonomies($post_type, 'names');
        }
...
if(!function_exists('etc_get_posts_of_grid')) {
    function etc_get_posts_of_grid($post_type = 'post', $atts = array(), $taxonomy = array(), $args_extra = array())
    {
        extract($atts);
        if (!empty($post_ids)) {
            $cms_query = new WP_Query(array(
                'post_type' => $post_type,
                'post__in' => array_map('intval', explode(',', $post_ids))
            ));
            $cms_paged = 1;
            $posts = $cms_query->query($cms_query->query_vars);
        } else {
            $args = array(
                'post_type' => $post_type,
...
//        }
        ...
        if (empty($source)) {
            $source_new = etc_get_grid_term_list($post_type, $taxonomy);
        ...
        $pagination = get_the_posts_pagination(array(
            'screen_reader_text' => '',
            'mid_size' => 2,
            'prev_text' => esc_html__('Back', ETC_TEXT_DOMAIN),
            'next_text' => esc_html__('Next', ETC_TEXT_DOMAIN),
        ));
        global $paged;
        $paged = $cms_paged;
...
if(!function_exists('etc_get_term_of_post_to_class')){
    function etc_get_term_of_post_to_class($post_id, $tax = array())
    {
        $term_list = array();
        foreach ($tax as $taxo) {
...
if(!function_exists('etc_get_all_page')){
    function etc_get_all_page(){
