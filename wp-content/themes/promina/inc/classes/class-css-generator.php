...
class CSS_Generator {
	...
	function __construct() {
		$this->opt_name = promina_get_opt_name();

		if ( empty( $this->opt_name ) ) {
			return;
		}
		$this->dev_mode = promina_get_opt( 'dev_mode', '0' ) === '1' ? true : false;
	...
	protected function options_output() {
		...
		$primary_color = promina_get_opt( 'primary_color', '#fe5a0e' );
		if ( ! promina_is_valid_color( $primary_color ) ) {
			$primary_color = '#fe5a0e';
		}
		printf( '$primary_color: %s;', esc_attr( $primary_color ) );

		$secondary_color = promina_get_opt( 'secondary_color', '#1b1a1a' );
		if ( ! promina_is_valid_color( $secondary_color ) ) {
			$secondary_color = '#1b1a1a';
		}
		printf( '$secondary_color: %s;', esc_attr( $secondary_color ) );

		$link_color = promina_get_opt( 'link_color', '#fe5a0e' );
		if ( ! empty( $link_color['regular'] ) && isset( $link_color['regular'] ) ) {
			printf( '$link_color: %s;', esc_attr( $link_color['regular'] ) );
		} else {
			echo '$link_color: #fe5a0e;';
		}

		$link_color_hover = promina_get_opt( 'link_color', '#fe5a0e' );
		if ( ! empty( $link_color['hover'] ) && isset( $link_color['hover'] ) ) {
			printf( '$link_color_hover: %s;', esc_attr( $link_color['hover'] ) );
		} else {
			echo '$link_color_hover: #fe5a0e;';
		}

		$link_color_active = promina_get_opt( 'link_color', '#fe5a0e' );
		if ( ! empty( $link_color['active'] ) && isset( $link_color['active'] ) ) {
			printf( '$link_color_active: %s;', esc_attr( $link_color['active'] ) );
		} else {
			echo '$link_color_active: #fe5a0e;';
		}
	...
	function enqueue() {
		$css = $this->inline_css();
		if ( ! empty( $css ) ) {
			wp_add_inline_style( 'promina-theme', $this->dev_mode ? $css : promina_css_minifier( $css ) );
		}
	}
	...
	protected function inline_css() {
		ob_start();
		/* Logo */
		$logo_maxh = promina_get_opt( 'logo_maxh' );
		...
		<?php $logo_maxh_sm = promina_get_opt( 'logo_maxh_sm' );
		...
		$sticky_menu_color = promina_get_opt( 'sticky_menu_color' );
		...
		/* Page Title */
		$ptitle_bg = promina_get_page_opt( 'ptitle_bg' );
		$title_font_size = promina_get_page_opt( 'title_font_size' );
		...
		/* Content */
		$content_bg_color = promina_get_page_opt( 'content_bg_color' );
		...
		/* Footer */
		$footer_bg_color_top      = promina_get_opt( 'footer_bg_color_top' );
		$footer_top_heading_color = promina_get_opt( 'footer_top_heading_color' );
		$footer_top_heading_fs    = promina_get_opt( 'footer_top_heading_fs' );
		$footer_top_heading_tt    = promina_get_opt( 'footer_top_heading_tt' );
		...
		/* Custom Css */
		$custom_css = promina_get_opt( 'site_css' );
		if ( ! empty( $custom_css ) ) {
