 ...
 * Header layout
 **/
function promina_page_loading()
{
    $page_loading = promina_get_opt( 'show_page_loading', false );
 ...
 * Header layout
 **/
function promina_header_layout()
{
    $header_layout = promina_get_opt( 'header_layout', '1' );
    $custom_header = promina_get_page_opt( 'custom_header', '0' );

    if ( $custom_header == '1' && is_page() || $custom_header == '1' && is_singular('project') || $custom_header == '1' && is_singular('service') )
    {
        $page_header_layout = promina_get_page_opt('header_layout');
 ...
 * Page title layout
 **/
function promina_page_title_layout()
{
 ...
 * Page title layout
 **/
function promina_footer()
{
    if(is_404()) {
        return true;
    }
    $footer_layout = promina_get_opt( 'footer_layout', '1' );
 ...
 */
function promina_primary_class( $sidebar_pos, $extra_class = '' )
{
 ...
 * @param  string $extra_class
 */
function promina_secondary_class( $sidebar_pos, $extra_class = '' )
{
...
function promina_breadcrumb()
{
    ...
    foreach ( $entries as $entry )
    {
        ...
        if ( ! empty( $entry['url'] ) )
        {
            ...
            if (strpos($entry['url'] , 'category') !== false) {


                $entry['url']=get_permalink( 86 );
                $entry['label']="Blog";
            }
            printf(
...
function promina_entry_link_pages()
{
    wp_link_pages( array(
...
if ( ! function_exists( 'promina_entry_excerpt' ) ) :
    /**
     * Print post excerpt based on length.
     *
     * @param  integer $length
     */
    function promina_entry_excerpt( $length = 55 )
    {
        $cms_the_excerpt = get_the_excerpt();
        if(!empty($cms_the_excerpt)) {
            echo esc_html($cms_the_excerpt);
        } else {
            echo wp_kses_post(promina_get_the_excerpt( $length ));
        }
    }
endif;
 ...
 */
function promina_entry_edit_link()
{
    edit_post_link(
        sprintf(
            wp_kses(
            /* translators: %s: Name of current post. Only visible to screen readers */
                esc_html__( 'Edit', 'promina' ),
                array(
                    'span' => array(
                        'class' => array(),
                    ),
...
if(!function_exists('promina_ajax_paginate_links')){
    function promina_ajax_paginate_links($link){
...
add_action( 'wp_ajax_promina_get_pagination_html', 'promina_get_pagination_html' );
add_action( 'wp_ajax_nopriv_promina_get_pagination_html', 'promina_get_pagination_html' );
if(!function_exists('promina_get_pagination_html')){
    function promina_get_pagination_html(){
        try{
            if(!isset($_POST['query_vars'])){
                throw new Exception(__('Something went wrong while requesting. Please try again!', 'promina'));
            }
            $query = new WP_Query($_POST['query_vars']);
            ob_start();
            promina_posts_pagination( $query,  true );
            $html = ob_get_clean();
            wp_send_json(
                array(
                    'status' => true,
                    'message' => esc_html__('Load Successfully!', 'promina'),
                    'data' => array(
                        'html' => $html,
 ...
 * @return void
 */
function promina_posts_pagination( $query = null, $ajax = false )
{
    if($ajax){
        add_filter('paginate_links', 'promina_ajax_paginate_links');
    }
 ...
 */
if ( ! function_exists( 'promina_archive_meta' ) ) :
    function promina_archive_meta() {
        $archive_author_on = promina_get_opt( 'archive_author_on', false );
        $archive_categories_on = promina_get_opt( 'archive_categories_on', true );
        $archive_comments_on = promina_get_opt( 'archive_comments_on', true );
        $archive_date_on = promina_get_opt( 'archive_date_on', true );
            ...
            <ul class="entry-meta">
                ...
                <?php if($archive_author_on) : ?>
                    <li class="item-author">
                        <span><?php echo esc_html__('By', 'promina'); ?></span>
                ...
                <?php if($archive_comments_on) : ?>
                    <li class="item-comment"><a href="<?php the_permalink(); ?>"><?php echo comments_number(esc_html__('No Comments', 'promina'),esc_html__('Comment: 1', 'promina'),esc_html__('Comments: %', 'promina')); ?></a></li>
                <?php endif; ?>
            </ul>
        <?php endif; }
endif;

if ( ! function_exists( 'promina_post_meta' ) ) :
    function promina_post_meta() {
        $post_author_on = promina_get_opt( 'post_author_on', true );
        $post_categories_on = promina_get_opt( 'post_categories_on', true );
        $post_date_on = promina_get_opt( 'post_date_on', true );
        $post_comments_on = promina_get_opt( 'post_comments_on', false );
...
if ( ! function_exists( 'promina_post_meta_event' ) ) :
    function promina_post_meta_event() {
 ...
 * Prints tag list
 */
if ( ! function_exists( 'promina_entry_tagged_in' ) ) :
    /**
     * Prints HTML with meta information for the current post-date/time.
     */
    function promina_entry_tagged_in()
    {
        ob_start();
        ?>
        <span><?php echo esc_html__('Tags:', 'promina');?></span>
        <?php
 ...
 */
function promina_socials_share_default() { ?>
    <div class="entry-socail">
        <a class="fb-social hover-effect" title="Facebook" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php the_permalink(); ?>">
            <i class="fa fa-facebook"></i><span><?php echo esc_html__('Share on Facebook','promina')?></span>
        </a>
        <a class="tw-social hover-effect" title="Twitter" target="_blank" href="https://twitter.com/home?status=<?php the_permalink(); ?>">
            <i class="fa fa-twitter"></i><span><?php echo esc_html__('Share on Twitter','promina')?></span>
        </a>
        <a class="g-social hover-effect" title="Google Plus" target="_blank" href="https://plus.google.com/share?url=<?php the_permalink(); ?>">
            <i class="fa fa-google"></i><span><?php echo esc_html__('Share on Google','promina')?></span>
        </a>
    </div>
    <?php
}
 ...
 * Footer Top
 */
function promina_footer_top() {
    $footer_top_column = promina_get_opt( 'footer_top_column' );
    $footer_layout = promina_get_opt( 'footer_layout' );
    $footer_contact_phone_label = promina_get_opt( 'footer_contact_phone_label' );
    $footer_contact_phone = promina_get_opt( 'footer_contact_phone' );
    $footer_contact_email_label = promina_get_opt( 'footer_contact_email_label' );
    $footer_contact_email = promina_get_opt( 'footer_contact_email' );
    ...
    for($i = 1 ; $i <= $footer_top_column ; $i++){
        if ( is_active_sidebar( 'sidebar-footer-' . $i ) ){
                ...
                <div class="footer5-section-group">
                    <?php if(!empty($footer_contact_phone) && !empty($footer_contact_email)) : ?>
                        <section id="footer5-quick-contact" class="widget">
                            <h2 class="footer-widget-title"><?php echo esc_html__('Quick Contact', 'promina'); ?></h2>
                    ...
                    <section id="footer5-social-contact" class="widget">
                        <h2 class="footer-widget-title"><?php echo esc_html__('Social', 'promina'); ?></h2>
                        <div class="footer-widget-content">
                            <div class="footer-social">
                                <?php promina_top_bar_social_icon_box(); ?>
                            </div>
                        </div>
                    </section>
                </div>
            ...
            if($i == '3' && $footer_layout == '6') { ?>
                <div class="footer5-section-group">
                    <section id="footer5-social-contact" class="widget">
                        <h2 class="footer-widget-title"><?php echo esc_html__('Follow Us!', 'promina'); ?></h2>
                        <div class="footer-widget-content">
                            <div class="footer-social">
                                <?php promina_top_bar_social_icon_box(); ?>
                            </div>
 ...
 * Related Post
 */
function promina_project_related_post()
{
    ...
    if($post_related_on) {
        ...
        if (count($query_similar->posts) > 1) {
            ...
            wp_enqueue_script( 'promina-carousel' );
            ?>
            <div class="cms-related-post">
                <h4 class="widget-title"><?php echo esc_html__('Related Posts', 'promina'); ?></h4>
                <div class="cms-related-post-inner owl-carousel" data-item-xs="1" data-item-sm="2" data-item-md="3" data-item-lg="3" data-item-xl="3" data-item-xxl="3" data-margin="30" data-loop="false" data-autoplay="false" data-autoplaytimeout="5000" data-smartspeed="250" data-center="false" data-arrows="false" data-bullets="false" data-stagepadding="0" data-stagepaddingsm="0" data-rtl="false">
                    <?php foreach ($query_similar->posts as $post):
                        ...
                        if (has_post_thumbnail(get_the_ID()) && wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), false)) :
                            $thumbnail_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'promina-medium', false);
                        endif;
                                ...
                                <div class="grid-item-inner">
                                    <?php if (has_post_thumbnail()) { ?>
                                        <div class="item-featured">
                                            <a href="<?php the_permalink(); ?>"><img alt="<?php echo esc_attr__('project image', 'promina');?>" src="<?php echo esc_url($thumbnail_url[0]); ?>" /></a>
                                        </div>
                                    <?php } ?>
                                    <h3 class="item-title">
            ...
            <div class="cms-modal-content">
                <form role="search" method="get" class="search-form-popup" action="<?php echo esc_url(home_url( '/' )); ?>">
                    <div class="searchform-wrap">
                        ...
                        <input type="text" placeholder="<?php echo esc_attr__('Type Words Then Enter', 'promina'); ?>" id="search" name="s" class="search-field" />
                    </div>
                </form>
            </div>
        </div>
 ...
 * Sidebar Hidden
 */
function promina_sidebar_hidden()
{
 ...
 * User custom fields.
 */
add_action( 'show_user_profile', 'promina_user_fields' );
add_action( 'edit_user_profile', 'promina_user_fields' );
function promina_user_fields($user){
 ...
 */
add_action( 'personal_options_update', 'promina_save_user_custom_fields' );
add_action( 'edit_user_profile_update', 'promina_save_user_custom_fields' );
function promina_save_user_custom_fields( $user_id )
{
...
/* Author Social */
function promina_get_user_social() {
...
function promina_social_share_product() { ?>
 ...
 * Social Icon
 */
function promina_social_header() {
    $h_social_facebook_url = promina_get_opt( 'h_social_facebook_url' );
    $h_social_twitter_url = promina_get_opt( 'h_social_twitter_url' );
    $h_social_inkedin_url = promina_get_opt( 'h_social_inkedin_url' );
    $h_social_instagram_url = promina_get_opt( 'h_social_instagram_url' );
    $h_social_google_url = promina_get_opt( 'h_social_google_url' );
    $h_social_skype_url = promina_get_opt( 'h_social_skype_url' );
    $h_social_pinterest_url = promina_get_opt( 'h_social_pinterest_url' );
    $h_social_vimeo_url = promina_get_opt( 'h_social_vimeo_url' );
    $h_social_youtube_url = promina_get_opt( 'h_social_youtube_url' );
    $h_social_yelp_url = promina_get_opt( 'h_social_yelp_url' );
    $h_social_tumblr_url = promina_get_opt( 'h_social_tumblr_url' );
    $h_social_tripadvisor_url = promina_get_opt( 'h_social_tripadvisor_url' );
...
function promina_social_footer() {
    $f_social_facebook_url = promina_get_opt( 'f_social_facebook_url' );
    $f_social_twitter_url = promina_get_opt( 'f_social_twitter_url' );
    $f_social_inkedin_url = promina_get_opt( 'f_social_inkedin_url' );
    $f_social_instagram_url = promina_get_opt( 'f_social_instagram_url' );
    $f_social_google_url = promina_get_opt( 'f_social_google_url' );
    $f_social_skype_url = promina_get_opt( 'f_social_skype_url' );
    $f_social_pinterest_url = promina_get_opt( 'f_social_pinterest_url' );
    $f_social_vimeo_url = promina_get_opt( 'f_social_vimeo_url' );
    $f_social_youtube_url = promina_get_opt( 'f_social_youtube_url' );
    $f_social_yelp_url = promina_get_opt( 'f_social_yelp_url' );
    $f_social_tumblr_url = promina_get_opt( 'f_social_tumblr_url' );
    $f_social_tripadvisor_url = promina_get_opt( 'f_social_tripadvisor_url' );
...
if(!function_exists('promina_get_post_grid_layout1')){
    function promina_get_post_grid_layout1($posts = [], $settings = []){
        extract($settings);
                        ...
                        <div class="entry-body">
                            ...
                            <?php if($show_button == 'true'): ?>
                                <div class="entry-readmore">
                                    <a class="btn-more elementor-animation-<?php echo esc_attr($hover_animation); ?>" href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><span><?php echo esc_attr($button_text); ?></span><i class="fac fac-arrow-right space-left"></i></a>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
...
if(!function_exists('promina_get_project_grid_layout1')){
    function promina_get_project_grid_layout1($posts = [], $settings = []){
        extract($settings);
                ...
                <div class="<?php echo esc_attr($item_class . ' ' . $filter_class); ?>">
                    <div class="grid-item-inner">
                        ...
                        <div class="entry-body">
                            ...
                            <?php if($show_button == 'true'): ?>
                                <div class="entry-readmore">
                                    <a class="btn-more elementor-animation-<?php echo esc_attr($hover_animation); ?>" href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo esc_attr($button_text); ?><i class="fac fac-arrow-right space-left"></i></a>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
...
if(!function_exists('promina_get_project_grid_layout2')){
    function promina_get_project_grid_layout2($posts = [], $settings = []){
        extract($settings);
        if($thumbnail_size != 'custom'){
            $img_size = $thumbnail_size;
        }
...
if(!function_exists('promina_get_service_layout1')){
    function promina_get_service_layout1($posts = [], $settings = []){
        extract($settings);
        if($thumbnail_size != 'custom'){
            $img_size = $thumbnail_size;
        }
        ...
        if (is_array($posts)):
            foreach ($posts as $post):
                ...
                <div class="<?php echo esc_attr($item_class . ' ' . $filter_class); ?>">
                    <div class="grid-item-inner">
                        <div class="cms-grid-overlay bg-image" style="background-image: url(<?php echo esc_url($overlay_bg); ?>);"></div>
                        <div class="entry-body">
...
if(!function_exists('promina_get_service_layout2')){
    function promina_get_service_layout2($posts = [], $settings = []){
        extract($settings);
        if($thumbnail_size != 'custom' && $thumbnail_size != 'full'){
            $img_size = $thumbnail_size;
        }
                ...
                <div class="<?php echo esc_attr($item_class . ' ' . $filter_class); ?>">
                    <div class="grid-item-inner">
                        ...
                        <?php endif; ?>
                            ...
                            <?php if($show_button == 'true'): ?>
                                <div class="entry-readmore">
                                    <a class="btn btn-secondary btn-small elementor-animation-<?php echo esc_attr($hover_animation); ?>" href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo esc_attr($button_text); ?><i class="fac fac-arrow-right space-left"></i></a>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
...
if(!function_exists('promina_get_service_layout3')){
    function promina_get_service_layout3($posts = [], $settings = []){
        extract($settings);
        if($thumbnail_size != 'custom' && $thumbnail_size != 'full'){
            $img_size = $thumbnail_size;
        }
...
if(!function_exists('promina_get_post_grid')){
    function promina_get_post_grid($posts = [], $settings = []){
        if (empty($posts) || !is_array($posts) || empty($settings) || !is_array($settings)) {
            return false;
        }
        switch ($settings['template_type']) {
            case 'post_grid_layout1':
                promina_get_post_grid_layout1($posts, $settings);
                break;

            case 'project_grid_layout1':
                promina_get_project_grid_layout1($posts, $settings);
                break;

            case 'project_grid_layout2':
                promina_get_project_grid_layout2($posts, $settings);
                break;

            case 'service_layout1':
                promina_get_service_layout1($posts, $settings);
                break;

            case 'service_layout2':
                promina_get_service_layout2($posts, $settings);
                break;

            case 'service_layout3':
                promina_get_service_layout3($posts, $settings);
                break;
...
add_action( 'wp_ajax_promina_load_more_post_grid', 'promina_load_more_post_grid' );
add_action( 'wp_ajax_nopriv_promina_load_more_post_grid', 'promina_load_more_post_grid' );
if(!function_exists('promina_load_more_post_grid')){
    function promina_load_more_post_grid(){
        try{
            ...
            ob_start();
            promina_get_post_grid($posts, $settings);
            $html = ob_get_clean();
            wp_send_json(
                array(
                    'status' => true,
...
*/
function promina_post_nav_default() {
    global $post;
    $previous = ( is_attachment() ) ? get_post( $post->post_parent ) : get_adjacent_post( false, '', true );
    $next     = get_adjacent_post( false, '', false );
        ...
        <div class="nav-links">
            <?php if ( is_a( $previous_post , 'WP_Post' ) && get_the_title( $previous_post->ID ) != '') { 
                ...
                <div class="nav-item nav-post-prev">
                    ...
                    <div class="nav-post-meta">
                        <label><?php echo esc_html__('Previous', 'promina'); ?></label>
            ...
            <?php if ( is_a( $next_post , 'WP_Post' ) && get_the_title( $next_post->ID ) != '') { 
                ...
                <div class="nav-item nav-post-next">
                    ...
                    <?php endif; ?>
                    <div class="nav-post-meta">
                        <label><?php echo esc_html__('Next', 'promina'); ?></label>
